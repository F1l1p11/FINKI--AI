...
class Snake (Problem):
    ...
    def h(self, node):
        pos_green, snake_position, _ = node.state
        sx, sy = snake_position[-1]
        total = 0
        if len(pos_green) > 0:
            px, py = pos_green[0]
            for (gx, gy) in pos_green:
                if (gx,gy) == (px,py):
    ...
    def successor(self, state):
        successor = {}
        pos_green, snake_position, direction = state
        for action, ((dx, dy), new_direction) in moves[direction].items():
            x, y = snake_position[-1]
            nx, ny = x + dx, y + dy

            new_s = list(snake_position)
            new_s = tuple(new_s)

            if 0 <= nx < self.size and 0 <= ny < self.size and (nx, ny) not in new_s:
                if (nx,ny) in pos_green:
                    ...
                    new_s.append((nx,ny))
                    new_s = tuple(new_s)
                    successor[action] = (new_g, new_s,new_direction)
                else:
                    new_s = list(new_s)
                    new_s.append((nx,ny))
                    new_s.remove(new_s[0])
                    new_s = tuple(new_s)
                    successor[action] = (pos_green, new_s,new_direction)
...
if __name__ == '__main__':
    ...
    initial = (pos_green,snake_position,"down")
