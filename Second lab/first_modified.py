...
class Robot(Problem):
    ...
    def goal_test(self, state):
        _,_,_,_,tmp1,tmp2,_,_ = state
        return tmp1 and tmp2

    def successor(self, state):
        x, y, c1, c2, M1_fixed, M2_fixed, repair, teleport_used = state
        successor = {}

        for d in dir:
            dx, dy = dir[d]
            nx, ny = (x + dx, y + dy)
            used_tp = teleport_used
            ...
            if (nx, ny) not in self.walls:
                ...
                successor[d] = (nx, ny, new_c1, new_c2, M1_fixed, M2_fixed, 0, used_tp)

        if (x, y) == self.m1_pos and not M1_fixed and len(c1) == len(self.m1_parts):
            if repair + 1 == self.m1_steps:
                successor["Repair"] = (x, y, c1, c2, True, M2_fixed, 0, teleport_used)
            else:
                successor["Repair"] = (x, y, c1, c2, M1_fixed, M2_fixed, repair + 1, teleport_used)

        elif (x, y) == self.m2_pos and M1_fixed and not M2_fixed and len(c2) == len(self.m2_parts):
            if repair + 1 == self.m2_steps:
                successor["Repair"] = (x, y, c1, c2, M1_fixed, True, 0, teleport_used)
            else:
                successor["Repair"] = (x, y, c1, c2, M1_fixed, M2_fixed, repair + 1, teleport_used)
...
if __name__ == '__main__':
    ...
    initial = (robot_start_pos[0], robot_start_pos[1], (), (), False, False, 0, False)
