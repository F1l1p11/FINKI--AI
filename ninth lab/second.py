...
if __name__ == '__main__':
    ...
    for inputs in inputs:
        ...
        split_thirds = len(train_X) // 3

        #split into 3 equal train x/y sets
        train_X_first = [row for row in train_X[split_thirds:split_thirds*2]] + [row for row in train_X[split_thirds*2:]]
        train_Y_first = [row for row in train_Y[split_thirds:split_thirds*2]] + [row for row in train_Y[split_thirds*2:]]
        test_X_first = [row for row in train_X[:split_thirds]]
        test_Y_first = [row for row in train_Y[:split_thirds]]

        train_X_second = [row for row in train_X[:split_thirds]] + [row for row in train_X[split_thirds*2:]]
        train_Y_second = [row for row in train_Y[:split_thirds]] + [row for row in train_Y[split_thirds*2:]]
        test_X_second = [row for row in train_X[split_thirds:split_thirds*2]]
        test_Y_second = [row for row in train_Y[split_thirds:split_thirds*2]]

        train_X_third = [row for row in train_X[:split_thirds]] + [row for row in train_X[split_thirds:split_thirds*2]]
        train_Y_third = [row for row in train_Y[:split_thirds]] + [row for row in train_Y[split_thirds:split_thirds*2]]
        test_X_third = [row for row in train_X[split_thirds*2:]]
        test_Y_third = [row for row in train_Y[split_thirds*2:]]

        for_training = {
            0: [train_X_first, train_Y_first, test_X_first, test_Y_first],
            1: [train_X_second, train_Y_second, test_X_second, test_Y_second],
            2: [train_X_third, train_Y_third, test_X_third, test_Y_third]
        }
        ...
        for i in range(3):
            train_X_set, train_Y_set, test_X_set, test_Y_set = for_training[i]

            scaler = MaxAbsScaler()
            train_X_scaled = scaler.fit_transform(train_X_set)
            test_X_scaled = scaler.transform(test_X_set)

            model = MLPClassifier(hidden_layer_sizes=(number_neurons,),learning_rate_init=0.001,max_iter=20,random_state=0)
            model.fit(train_X_scaled, train_Y_set)
            regular_accuracy = model.score(test_X_scaled, test_Y_set)
            ...
            if regular_accuracy > best_regular_accuracy:
                ...
                best_regular_classifier = model
                best_regular_scaler = scaler

            ensemble_classifier_1 = MLPClassifier(hidden_layer_sizes=(5,),learning_rate_init=0.001,max_iter=20,random_state=0)
            ensemble_classifier_2 = MLPClassifier(hidden_layer_sizes=(10,),learning_rate_init=0.001,max_iter=40,random_state=0)

            ensemble_classifier_1.fit(train_X_scaled, train_Y_set)
            ensemble_classifier_2.fit(train_X_scaled, train_Y_set)
            ...
            for k in range(len(test_Y_set)):
                ...
                if predicted_class == test_Y_set[k]:
                    correct += 1

            ensemble_accuracy = correct / len(test_Y_set)
        ...
        if average_regular >= average_ensemble:
            ...
            test_X = best_ensemble_scaler.transform(test_X)
            model1, model2 = best_ensemble

            probabilities_1 = model1.predict_proba(test_X)
            probabilities_2 = model2.predict_proba(test_X)

            correct_test = 0
            for k in range(len(test_Y)):
                if probabilities_1[k][probabilities_1[k].argmax()] > probabilities_2[k][probabilities_2[k].argmax()]:
                    predicted_class = model1.predict([test_X[k]])
                else:
                    predicted_class = model2.predict([test_X[k]])
