...
if __name__ == '__main__':
    ...
    new_dataset = []
    for row in dataset:
        new_row = [element for index,element in enumerate(row) if index != col_index]
        new_dataset.append(new_row)
    new_sample = [element for index,element in enumerate(sample) if index != col_index]

    split = int(len(new_dataset) * 0.8)
    ...
    model_1 = MLPClassifier(hidden_layer_sizes=(5,),random_state=0,max_iter=500)
    model_1.fit(train_X, train_Y)
    predictions_1 = model_1.predict(test_X)

    model_2 = MLPClassifier(hidden_layer_sizes=(10,),random_state=0,max_iter=500)
    model_2.fit(train_X, train_Y)
    predictions_2 = model_2.predict(test_X)

    accuracy_1 = sum (1 for guess,true in zip(predictions_1,test_Y) if guess == true) / len(test_Y)
    accuracy_2 = sum(1 for guess, true in zip(predictions_2, test_Y) if guess == true) / len(test_Y)
    predict_class = model_1.predict([new_sample])[0]
    predict_probability = model_1.predict_proba([new_sample])[0]

    print (f'Tochnost with 5 neurons: {accuracy_1}')
    print (f'Tochnost with 10 neurons: {accuracy_2}')
    print (f'Predvidena klasa: {predict_class}')
    print (f'Verojatnosti: {predict_probability}')