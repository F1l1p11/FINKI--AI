...
if __name__ == '__main__':
    ...
    model = MLPClassifier (    hidden_layer_sizes=(number_neurons,), random_state=0,learning_rate_init=0.001,max_iter=500,activation='relu')
    model.fit(train_X, train_Y)
    prediction_1 = model.predict(test_X)

    accuracy = sum (1 for v,p in zip(test_Y,prediction_1) if v == p ) / len(test_Y)
    print(f'Tochnost: {accuracy}')

    predict = model.predict([new_sample])[0]
    print(f'Predvidena klasa: {predict}')

    probability = model.predict_proba([new_sample])[0]
    print(f'Verojatnosti: {probability}')
