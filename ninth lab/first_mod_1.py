...
if __name__ == '__main__':
    ...
    for row in dataset:
        ...
        even_features = [x for x in features if int(x) % 2 == 0]

        max_len = max(max_len, len(even_features))

        new_dataset.append([even_features, label])

    new_sample = [x for x in sample if int(x) % 2 == 0]
    max_len = max(max_len, len(new_sample))

    final_dataset = []

    for features, label in new_dataset:
        while len(features) < max_len:
            features.append(2)

        final_dataset.append(features + [label])

    while len(new_sample) < max_len:
        new_sample.append(2)

    split = int(len(final_dataset) * 0.8)

    train_X = [row[:-1] for row in final_dataset[:split]]
    train_Y = [row[-1] for row in final_dataset[:split]]

    test_X = [row[:-1] for row in final_dataset[split:]]
    test_Y = [row[-1] for row in final_dataset[split:]]

    model = MLPClassifier(hidden_layer_sizes=(hidden_neurons,),random_state=0,max_iter=500)

    model.fit(train_X, train_Y)
    predictions = model.predict(test_X)

    accuracy = sum(1 for p, t in zip(predictions, test_Y) if p == t) / len(test_Y)
    predict_class = model.predict([new_sample])[0]
    predict_probability = model.predict_proba([new_sample])[0]
