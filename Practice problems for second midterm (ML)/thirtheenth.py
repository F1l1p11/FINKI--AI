...
if __name__ == '__main__':
    ...
    X_0 = [row[:-1] for row in dataset if int(row[-1]) == 0]
    X_1 = [row[:-1] for row in dataset if int(row[-1]) == 1]
    X_2 = [row[:-1] for row in dataset if int(row[-1]) == 2]

    Y_0 = [row[-1] for row in dataset if int(row[-1]) == 0]
    Y_1 = [row[-1] for row in dataset if int(row[-1]) == 1]
    Y_2 = [row[-1] for row in dataset if int(row[-1]) == 2]

    #NB
    train_X_NB = X_0[:int(len(X_0) * x1)] + X_1[:int(len(X_1) * x1)] + X_2[:int(len(X_2) * x1)]
    train_Y_NB = Y_0[:int(len(Y_0) * x1)] + Y_1[:int(len(Y_1) * x1)] + Y_2[:int(len(Y_2) * x1)]

    #DTC
    train_X_DTC = X_0[int(len(X_0) * x1):int(len(X_0) * x2)] + X_1[int(len(X_1) * x1):int(len(X_1) * x2)] + X_2[int(len(X_2) * x1):int(len(X_2) * x2)]
    train_Y_DTC = Y_0[int(len(Y_0) * x1):int(len(Y_0) * x2)] + Y_1[int(len(Y_1) * x1):int(len(Y_1) * x2)] + Y_2[int(len(Y_2) * x1):int(len(Y_2) * x2)]

    #RFC
    train_X_RFC = X_0[:int(len(X_0) * x2)] + X_1[:int(len(X_1) * x2)] + X_2[:int(len(X_2) * x2)]
    train_Y_RFC = Y_0[:int(len(Y_0) * x2)] + Y_1[:int(len(Y_1) * x2)] + Y_2[:int(len(Y_2) * x2)]

    test_X = X_0[int(len(X_0) * x2):] + X_1[int(len(X_1) * x2):] + X_2[int(len(X_2) * x2):]
    test_Y= Y_0[int(len(Y_0) * x2):] + Y_1[int(len(Y_1) * x2):] + Y_2[int(len(Y_2) * x2):]
    ...
    classifier_DTC.fit(train_X_DTC, train_Y_DTC)
    classifier_RFC.fit(train_X_RFC, train_Y_RFC)

    predictions_NB = classifier_NB.predict(test_X)
    predictions_DTC = classifier_DTC.predict(test_X)
    predictions_RFC = classifier_RFC.predict(test_X)

    correct_predictions = 0
    for NB,DTC,RFC,correct in zip(predictions_NB,predictions_DTC,predictions_RFC,test_Y):
        temp = 0
        if NB == correct:
            temp += 1
        if DTC == correct:
