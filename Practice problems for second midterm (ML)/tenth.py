...
if __name__ == '__main__':
    ...
    # transform target column to binary classes
    for row in dataset:
        if row[-1] >= 5:
            row[-1] = 1
        else:
    ...
    train_X_standard = scaler_standard.fit_transform(train_X_new)
    test_X_standard = scaler_standard.transform(test_X_new)

    # MinMaxScaler
    scaler_minmax = MinMaxScaler()

    train_X_minmax = scaler_minmax.fit_transform(train_X_new)
    test_X_minmax = scaler_minmax.transform(test_X_new)

    # Neural network
    model_standard = MLPClassifier(
    ...
    # train models
    model_standard.fit(train_X_standard, train_Y)
    model_minmax.fit(train_X_minmax, train_Y)

    # predictions
    pred_standard = model_standard.predict(test_X_standard)
    pred_minmax = model_minmax.predict(test_X_minmax)

    # accuracy
    acc_standard = sum (1 for value,prediction in zip(pred_standard, test_Y) if value == prediction)/len(test_Y)
    acc_minmax = sum (1 for value,prediction in zip(pred_minmax, test_Y) if value == prediction)/len(test_Y)

    print(f'Tocnost so StandardScaler: {acc_standard}')
